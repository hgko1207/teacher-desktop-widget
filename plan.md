...
| Phase 4 Step 1~3 | 파일 드래그&드롭 + 아이콘 + 열기 + 우클릭 + 저장 | ✅ 완료 |
...
### 12-3. 메모 위젯 업그레이드 (핸드폰 메모 스타일)

**현재**: 단순 textarea 1개 (한 장의 메모)
**목표**: 핸드폰 메모 앱처럼 여러 메모 관리
...
| 순서 | 항목 | 이유 |
|------|------|------|
| **1** | 12-2. 학사 일정 | KEY 불필요, 바로 구현 가능, 교사 필수 |
| **2** | 12-3. 메모 업그레이드 | UX 크게 개선, 복잡도 중간 |
| **3** | 12-1. 미세먼지 | API 키 필요하지만 중요한 정보 |
| **4** | 12-4. 알림 | 부가 기능, Electron API 활용 |
| 5 | 12-5. 바탕화면 고정 | 보류 |
